...
Dirty.prototype.addIndex = function(name, indexFn) {
    this._indexFns[name] = {indexFn: indexFn, keys: {}};
};

Dirty.prototype._updateIndexes = function(key, newVal) {
   for (idxKey in this._indexFns) {
       var indexFn = this._indexFns[idxKey].indexFn;
       var keys = this._indexFns[idxKey].keys;
       if (this._docs[key]) {
           var oldIdxKey = indexFn(key, this._docs[key]);
        }
       if (newVal) {
           var newIdxKey = indexFn(key, newVal);
       };
       if ((oldIdxKey) && (oldIdxKey === newIdxKey)) {
           continue;
       };
       if (oldIdxKey) {
       oldCol = keys[oldIdxKey];
       oldCol.splice(oldCol.indexOf(key), 1);
       if (oldCol.length == 0) delete keys[oldIdxKey];       
   }
       if (newIdxKey) {
           if (!keys[newIdxKey]) keys[newIdxKey] = [];
           keys[newIdxKey].push(key);
       }
   };
};

Dirty.prototype.find = function(name, value) {
    var self = this;
    var validKeys = this._indexFns[name].keys[value];
    return !validKeys ? [] :
        this._indexFns[name].keys[value].map(function(k){
