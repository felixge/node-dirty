...
Dirty.prototype.set = function(key, doc, cb) {
  ...
  if (this._keys[key] === undefined || this._keys[key]._deleted) {
    this.length++;
    // this._docs.splice(this._keys[key]);
  }
...
Dirty.prototype.remove = function(key, cb) {
  var
    self = this,
    currentIndex = this._keys[key];

  this.length--;
  this.set(key, {_deleted: true}, function() {
    self._docs.splice(self._keys[key]);
    self._docs.splice(currentIndex);
