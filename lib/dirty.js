...
Dirty.prototype.set = function(key, doc, cb) {
  ...
  if (!this.flushTimer && this.memoryQueueLength !== this.flushLimit) {
    var self = this;
    this.flushTimer = setInterval(function() {
      self.flush();
    }, this.flushInterval);
...
Dirty.prototype.flush = function(cb) {
  ...
  while (this.memoryQueueLength > 0) {
    this.memoryQueueLength--;
    chunk += JSON.stringify(this.docs[length-this.memoryQueueLength-1])+"\n";
  }

  this.flushQueueLength += this.flushLimit;
  return this.file.write(chunk).addCallback(function() {
    self.flushQueueLength -= self.flushLimit;
    // If memory === disk, emit the flush event
    if (self.flushQueueLength === 0) {
